...
serve(async (req) => {
  ...
  try {
    ...
    if (!cvBase64 && !cvText) {
      return respond(false, { error: "A CV (PDF or text) is required" });
    }
...
Return a single overall score (weighted average) and categorized feedback using the provided tool. Be encouraging while still providing actionable improvements.`;
    ...
    if (!response.ok) {
      ...
      return respond(false, { error: "AI audit failed. Please try again." });
    }
    ...
    if (!toolCall?.function?.arguments) {
      ...
      return respond(false, { error: "AI did not return structured audit" });
    }

    const audit = JSON.parse(toolCall.function.arguments);
    return respond(true, { data: audit });
  } catch (e) {
